...
const AnimTBody = (props) => (
  <TableContext.Consumer>
    {(isPage) => (
      <TweenOneGroup
        component="tbody"
        enter={isPage ? pageEnterAnim : enterAnim}
        leave={isPage ? pageLeaveAnim : leaveAnim}
        appear={false}
        exclusive
        {...props}
      />
...
const UsersTable = ({ className = 'table-enter-leave-demo' }) => {
  ...
  const fetchUsers = async (page = 1, pageSize = 10) => {
    setLoading(true);
    try {
      const { data: res } = await httpInterceptedServices(
        `/users/sieve?page=${page}&size=${pageSize}`,
        { method: 'POST' }
      );
      console.log({ res });
  ...
  const save = async (id) => {
    try {
      ...
      console.log({ row });
      await httpInterceptedServices(`/users/${id}`, {
        method: 'PUT',
        data: row,
      });
      fetchUsers(pagination.current, pagination.pageSize);
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      console.log({ values });
      await httpInterceptedServices('/users', {
        method: 'POST',
        data: addForm.getFieldsValue(),
      });
  ...
  const handleDelete = async (id) => {
    try {
      await httpInterceptedServices(`/users/${id}`, { method: 'DELETE' });
    ...
    <MainLayout>
      <div className={`${className}-table-wrapper w-full`} style={{ width: '100%' }}>
        <div className={`${className}-action-bar`}>
          <Button type="primary" onClick={openAddModal} disabled={editingKey !== ''}>
            Add
          </Button>
        </div>
